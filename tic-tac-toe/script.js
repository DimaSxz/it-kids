...
function startGame(settings) {
  const { size, playWithComputer, playerSide } = settings;

  // Если выбран режим игры с компьютером, обновляем настройки.
  if (playWithComputer) {
    pvpMode = false;
...
function doComputerMove() {
  ...
  // Выбираем случайную ячейку из свободных.
  const randomCell = uncheckedCells[Math.floor(Math.random() * uncheckedCells.length)];
  randomCell.click(); // Имитируем клик по ячейке.
  boardCover.style.display = 'none'; // Скрываем "покрывало".
}

async function showResultMessage(message) {
  return new Promise((resolve) => {
    setTimeout(() => {
      if (confirm(message)) {
        resolve(true); // Пользователь согласен.
      } else {
        resolve(false); // Пользователь отказывается.
      }
    }, 50); // Добавляем небольшую задержку перед показом confirm.
  });
}
